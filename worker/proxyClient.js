// PROXY CLIENT
//
// The proxy code (see "BEGIN EMSCRIPTEN PROXY CODE") is generated by
// Emscripten as of 2.0.11. This code was last substantively updated in
// 2018 (emscripten-core/emscripten@cd42e9d), so it is reasonable to
// retain a copy here and modify it.
//
// Update this code upon changes to the proxy script in Emscripten.
// See emscripten-core/emscripten/emcc.py --> worker_js_script()

////////////////////////////////////////////////////////////////////////
// PREAMBLE TO PROXY OODE
////////////////////////////////////////////////////////////////////////

export async function initializeProxy(
  filename, initialModule,
  _componentElement, _canvasElement, _consoleElement,
  userOptions
) {

var Module = initialModule || {};
...
Module['onCustomMessage'] = function(event) {
  ...
  switch(command) {
    case 'postRun':
      readyPromiseResolve(proxy);
      break;

    case 'calledMain':
      calledMainPromiseResolve();
      break;
    ...
    case 'pausedMainLoop':
      pausedMainLoopPromiseResolve();
      break;

    case 'resumedMainLoop':
      resumedMainLoopPromiseResolve();
      break;
...
var calledMainPromiseResolve;
function callMain(args) {
  let prom = new Promise(function(resolve) {
    calledMainPromiseResolve = resolve;
  });
...
var abortedPromiseResolve;
function abort(reason) {
  let prom = new Promise(function(resolve) {
    abortedPromiseResolve = resolve;
  });
...
var pausedMainLoopPromiseResolve;
function pauseMainLoop() {
  let prom = new Promise(function(resolve) {
    pausedMainLoopPromiseResolve = resolve;
  });

  postCustomMessage({
    command: 'pauseMainLoop'
  });

  return prom;
}

var resumedMainLoopPromiseResolve;
function resumeMainLoop() {
  let prom = new Promise(function(resolve) {
    resumedMainLoopPromiseResolve = resolve;
  });

  postCustomMessage({
    command: 'resumeMainLoop'
  });
...
// POSTAMBLE
//
// Re-implement event listeners here. For necessity, see
// https://github.com/emscripten-core/emscripten/issues/3572
//
// TODO:
// This procedure assumes that SDL2 is being used. To only add listeners
// when necessary, a better approach would be to send a postMessage()
// from the worker to this client to add a handler on this client-side.
// Maybe patch the worker's JSEvents.registerOrRemoveHandler().
////////////////////////////////////////////////////////////////////////

function cloneObject(event) {
  var ret = {};
  for (var x in event) {
    // This doesn't let us clone numeric keys
    //if (x == x.toUpperCase()) continue;
    var prop = event[x];
    if (isPrimitive(prop))
      ret[x] = prop;
    else if (isDomNode(prop))
      ret[x] = cloneDomNode(prop);
    // For performance reasons, don't clone arrays because we don't use them
    // else if (Array.isArray(prop))
    //   ret[x] = cloneArray(prop);
    else if (isClonable(prop))
      ret[x] = cloneObject(prop);
  }
  return ret;
}
...
function cloneDomNode(node) {
  // Only copy the properties that Emscripten uses in HTMLElements in events.
...
// See emscripten-ports/SDL2/src/video/emscripten/SDL_emscriptenevents.c
//   --> Emscripten_RegisterEventHandlers()
let eventMap = {
  'mousemove': { clientElement: _canvasElement, workerElement: 'canvas' },

  'mousedown': { clientElement: _canvasElement, workerElement: 'canvas' },
  'mouseup': { clientElement: document, workerElement: 'document' },

  'mouseenter': { clientElement: _canvasElement, workerElement: 'canvas' },
  'mouseleave': { clientElement: _canvasElement, workerElement: 'canvas' },

  // may be mousewheel if MIN_IE_VERSION <= 8 || MIN_SAFARI_VERSION < 60100
  'wheel': { clientElement: _canvasElement, workerElement: 'canvas' },

  'focus': { clientElement: window, workerElement: 'window' },
  'blur': { clientElement: window, workerElement: 'window' },

  'touchstart': { clientElement: _canvasElement, workerElement: 'canvas' },
  'touchend': { clientElement: _canvasElement, workerElement: 'canvas' },
  'touchmove': { clientElement: _canvasElement, workerElement: 'canvas' },
  'touchcancel': { clientElement: _canvasElement, workerElement: 'canvas' },

  // pointerlockchange
  // Not supported in --proxy-to-worker mode, see
  // emscripten-core/emscripten/src/library_html5.js --> emscripten_set_pointerlockchange_callback_on_thread()

  // \todo: Can be configured by SDL_SetHint(SDL_HINT_EMSCRIPTEN_KEYBOARD_ELEMENT), how to handle?
  'keydown': { clientElement: window, workerElement: 'window' },
  'keyup': { clientElement: window, workerElement: 'window' },
  'keypress': { clientElement: window, workerElement: 'window' },

  // fullscreenchange
  // may be mozfullscreenchange/webkitfullscreenchange/MSFullscreenChange
  'fullscreenchange': { clientElement: document, workerElement: 'document' },

  'resize': { clientElement: window, workerElement: 'window' },

  'visibilitychange': { clientElement: document, workerElement: 'document' },
  'beforeunload': { clientElement: window, workerElement: 'window' },
}

if (userOptions.captureFocusOnComponent) {
  eventMap.keydown.clientElement = _componentElement;
  eventMap.keyup.clientElement = _componentElement;
  eventMap.keypress.clientElement = _componentElement;
}
...
Object.keys(eventMap).forEach(function(eventName) {
  const targets = eventMap[eventName];
  const checkPreventDefault = (eventName === 'keydown');
  const isKey = (eventName.startsWith('key'));
  const isMouse = (eventName.startsWith('mouse') || eventName.includes('wheel'));
  const isResize = (eventName.startsWith('resize'));
  let capture = isMouse;

  targets.clientElement.addEventListener(eventName, function(event) {
    if (isKey && !userOptions.captureTabKey && event.keyCode === 9)
      return;

    if (isResize && userOptions.resizeCanvasOnElementSizing
        && event.relatedTarget !== _canvasElement)
      return;

    worker.postMessage({ target: targets.workerElement, event: cloneObject(event) });

    if (!checkPreventDefault || shouldPreventDefault(event))
